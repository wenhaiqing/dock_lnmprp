...
class Product extends Base
{
	...
	public function test_add_p()
	{
		...
		$category = $this->ProductModel()->db->where('name', '内存')->getOne('ln_categories');

		foreach ($productData as $data) {
			// 创建一个新商品
			$data1['title'] = $data['title'];
			$data1['long_title'] = $data['long_title'];
			$data1['description'] = $data['description'];
			$data1['image'] = $data['image'];
			$data1['price'] = $data['price'];
			$data1['on_sale'] = true;
			$data1['rating'] = 5;
			$data1['category_id'] = $category['id'];
			$product = $this->addProduct($data1);
			// 遍历商品数据中的 SKU 字段
			foreach ($data['skus'] as $sku) {
				$this->ProductModel()->db->insert('ln_product_skus',array_merge($sku, ['stock' => 999,'product_id'=>$product]));
