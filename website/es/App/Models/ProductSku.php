...
class ProductSku extends Base
{
	...
	public function desStock(int $id,int $amount)
	{
		...
		$order = $this->db->rawQuery("SELECT  stock FROM ln_product_skus WHERE  id = '".$id."' LIMIT 1 lock in share mode");
		Logger::getInstance()->log($order[0]['stock']);
		if ($order[0]['stock']>$amount){
...
//				->update($this->tableName,['stock'=>$this->db->dec($amount)]);
			...
			Logger::getInstance()->log($res.'-'.$this->db->getLastQuery());
			if (!$res){
